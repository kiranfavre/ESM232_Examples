...
sager = read.table("Data/sager.txt",
                   header=T)
head(sager)

# add date
sager = sager %>% mutate(date = paste(day,month,year, sep="/")) #adding date col
sager$date = as.Date(sager$date,"%d/%m/%Y")  #changing date format
...
```{r}
flow = read.table("Data/sager.txt",
                   header=T)

flow = flow %>% mutate(date = paste(day,month,year, sep="/")) #adding date col
flow$date = as.Date(sager$date,"%d/%m/%Y")
...
**We are interested in the difference in number of days the flow rate is above the 90% percentile of observations, which is 3.029, in the winter season (December - March) between the model and observations.**
...
- inputs: 
    - flow = data frame with model and obs flows with corresponding times 
    - threshold (z) = 3.028808 
- outputs: metric we are interest in... number of days that are above the extreme flow threshold that we determine

```{r}
days_model = flow %>% 
    filter(month %in% c("1","2", "3", "12")) %>% 
    select(model, month, year) %>% 
  filter(model > 3.028808) %>% 
  nrow()
...
```{r}
##think about: should we include a calculation for the threshold within the function

compute_high_day_metrics <- function(days_model, days_obs, z = 3.028808){
  #calculate # of days that model predicted above 90th percentile
 days_model = flow %>% 
    filter(month %in% c("1","2", "3", "12")) %>% 
    select(model, month, year) %>% 
  filter(model > 3.028808) %>% 
  ...
  #calculate # of days that observations were above 90th percentile 
   days_obs = flow %>% 
    filter(month %in% c("1","2", "3", "12")) %>% 
    select(obs, month, year) %>% 
  filter(obs > 3.028808) %>% 
  nrow()
  
  return(days_model - days_obs)
}

#subtract days above 90th percentile in model - days above 90th percentile in obs
```


# Using multiple metrics. 

**Second matrix: Number of flow rates exceeding the 90th percentile of observations in a given year.**

```{r}
#testing a function for finding the number of flow rates exceeding the 90th percentile of observations in a given year
compute_high_yr_metrics <- function(yr_model, yr_obs, z = 3.028808){
  #calculate # of days that model predicted above 90th percentile
 yr_model = flow %>% 
    filter(month %in% c("1","2", "3", "12")) %>% 
    select(model, month, year) %>% 
